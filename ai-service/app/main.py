...
async def process_with_paths(request_data: dict):
    """
    Process images using file paths (called by Next.js backend).
    
    This endpoint is for internal use by the Next.js backend.
    It receives file paths instead of uploaded files.
    """
    ...
    try:
        ...
        # Process synchronously (Next.js handles async)
        process_job(job)
        
        # Return result
        if job.status == JobStatus.DONE:
            # Return relative path for Next.js to find
